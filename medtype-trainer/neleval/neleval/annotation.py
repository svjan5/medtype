...
class Annotation(object):
	...
	* :
	...
	def compare_spans(self, other):
		...
		if   self.docid != other.docid:					return 'different documents'
		if   self.start  > other.end    or self.end  < other.start:	return 'non-overlapping'
		elif self.start == other.start and self.end == other.end: 	return 'duplicate'
		elif self.start  < other.start and self.end >= other.end: 	return 'nested'
		elif self.start >= other.start and self.end  < other.end: 	return 'nested'
...
class Candidate(object):
	...
	def __unicode__(self):
		...
		return u'{}\t{}\t{}'.format(self.eid, self.score or '', type_)
...
class Measure(object):
	...
	def build_index(self, annotations, key_fields=None, multi=False):
		...
		if multi:
			...
			return {tuple(getattr(ann, field) for field in key): ann for ann in annotations}
	...
	# Self-implemented
	def intersect(self, gold, system):
		kb_ind = self.key.index('kbid')
		tp = 0
		system = {x: 1 for x in system}

		for ele in gold:
			if '|' not in ele[kb_ind]:
				if ele in system: tp += 1
			else:
				for kbid in ele[kb_ind].split('|'):
					cpy_ele		= list(ele)
					cpy_ele[kb_ind]	= kbid
					cpy_ele 	= tuple(cpy_ele)
					if cpy_ele in system:
						tp += 1
						break
		return tp
	...
	def count_matches(self, system, gold):
		if self.is_clustering:
			raise ValueError('count_matches is inappropriate ''for {}'.format(self.agg))
		if not self.weighting:
			gold_index = self.build_index(gold)
			pred_index = self.build_index(system)

			if 'kbid' in self.key:
				tp = self.intersect(keys(gold_index), keys(pred_index))
			else:
			...
			key = [f for f in self.key if f not in self.weighting]
			weighting_fields = [f for f in self.key if f in self.weighting]
			...
			if (
				...
				raise NotImplementedError('No weighting support where annotations may have duplicate key')
			tp = 0.
			for k, (gold_ann,) in gold_index.items():
				try:
					...
					continue
				tp += self.calc_match_weight(gold_ann, pred_ann, weighting_fields)

			fn = sum(self.calc_match_weight(gold_ann, gold_ann, weighting_fields) for gold_ann, in gold_index.values()) - tp
			fp = sum(self.calc_match_weight(pred_ann, pred_ann, weighting_fields) for pred_ann, in pred_index.values()) - tp
		return tp, fp, fn

	def calc_match_weight(self, gold_ann, pred_ann, weighting_fields):
		w = 1
		for field in weighting_fields:
			w *= self.weighting[field](getattr(gold_ann, field), getattr(pred_ann, field))
		return w
		...
		* fn [(item, None), ...]
		...
		if self.is_clustering:
			raise ValueError('get_matches is inappropriate '
							 'for {}'.format(self.agg))
		if self.weighting:
			raise NotImplementedError('get_matches not implemented with weighting')
		gold_index = self.build_index(gold)
		pred_index = self.build_index(system)
	...
	# Self-implemented
	def pop(self, gold, key):
		res = []
		for k in self.gold_map[key]:
			res += gold[k]
		return res
	...
	def get_overlapping(self, system, gold):
		...
		if 'kbid' in self.key:
			self.gold_map = ddict(list)
			for kbid, docid in gold.keys():
				for kb in kbid.split('|'):
					self.gold_map[(kb, docid)].append((kbid, docid))


		for key, sys_annots in system.items():
			if 'kbid' in self.key:
				gold_annots = self.pop(gold, key)
			else:
				# gold_annots = gold.pop(key, [])
				gold_annots = gold[key]
			sys_annots.sort()
			gold_annots.sort()
		...
		return ({k: list(reversed(v)) for k, v in overlaps_sys.items()}, {k: list(reversed(v)) for k, v in overlaps_gold.items()})
	...
	def contingency(self, system, gold):
		if self.is_clustering:
			...
			fp, fn = self.count_overlap(system, gold, params[:3], params[3:])
