...
class Evaluate(object):
	...
	def __call__(self, measures=None):
		...
		system_annotations	= [ann for doc in self.system for ann in doc.annotations]
		gold_annotations	= [ann for doc in self.gold for ann in doc.annotations]
		...
		if not self.group_by:
			...
			get_group = lambda ann: tuple(getattr(ann, field) for field in self.group_by)
		...
		for measure in measures:
			...
			for group in itertools.product(*group_vals):
				...
				mat = Matrix(*get_measure(measure, weighting=self.weighting).
							 contingency(system_grouped[group], gold_grouped[group])
							 )
				measure_mats.append((group, mat))

				if not self.group_by or not self.overall:
					name = name_fmt.format(measure=measure, group=[json.dumps(v) for v in group])
	...
	def count(cls, measure, doc_pairs, weighting=None):
		...
		for sdoc, gdoc in doc_pairs:
			per_doc.append(Matrix(*measure.contingency(sdoc.annotations, gdoc.annotations)))
...
class Matrix(object):
	...
	def __str__(self):
		return 'ptp={},fp={},rtp={},fn={}'.format(self.ptp, self.fp, self.rtp, self.fn)
